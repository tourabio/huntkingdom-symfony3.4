    ...
    public void show(String titre, String texte){
        try {
            ...
            Notifications notificationBuilder = Notifications.create().title(titre).text(texte)
                    .graphic(new ImageView(img)).hideAfter(Duration.seconds(5)).position(Pos.BOTTOM_RIGHT).hideCloseButton();
            notificationBuilder.show();
        } catch (FileNotFoundException ex) {
            Logger.getLogger(ShowNotificationsAdmin.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
